...
@Component({
  selector: 'acc-list-treasury',
  templateUrl: './list-treasury.component.html',
  styleUrls: ['./list-treasury.component.scss']
})
export class ListTreasuryComponent extends PagedComponent implements OnInit, OnDestroy {
  treasuries: TreasuryPopulated[];
  rights: {[key: string]: boolean};
  columns: TableColumn[] = [
    { name: '_id', label: 'ID', sortable: false, filter: true },
    { name: 'name', label: 'TREASURY_NAME', sortable: true, filter: true },
    ...
    { name: 'currentBalance', label: 'CURRENT_BALANCE', sortable: true, filter: true },
  ...
  loadData() {
    ...
    this.treasuriesService.getTreasuries({
      limit: this.limit,
      skip: this.skip,
      ...
      .subscribe(
        data => {
          ...
          this.treasuriesService.setAppRights(data.rights);
  ...
  confirmDeleteTreasury(treasury: Treasury, index: number) {
    ...
    dialogRef.afterClosed().subscribe(
      (confirm) => {
        if (confirm) {
          this.deleteTreasury(treasury, index);
        }
      }
    );
  }
  deleteTreasury(treasury: Treasury, index: number) {
    this.beforeLoadingData();
    this.treasuriesService.deleteTreasury(treasury)
      .subscribe(
        (data) => {
          this.afterLoadingData();
          this.treasuries = [
            ...this.treasuries.slice(0, index),
            ...this.treasuries.slice(index + 1)
          ];
          this.toastService
